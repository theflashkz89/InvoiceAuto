...
def run_main_process(base_dir, log_output, booking_list_path=None, price_list_path=None):
    ...
    参数:
        ...
        booking_list_path (str, optional): Booking List 文件路径，用于客户信息核对
        price_list_path (str, optional): Price List 文件路径，用于自动查价
    """
    ...
    try:
        ...
        # 定义 info.xlsx 路径（无论是否有数据都需要定义，用于后续步骤）
        ...
        # ==================== 额外步骤：客户信息核对 (RPA) ====================
        if booking_list_path and os.path.exists(booking_list_path):
            print(f"\n【额外步骤】检测到 Booking List，开始执行客户核对...")
            print(f"Booking List 路径: {booking_list_path}")
            
            # info.xlsx 的路径在上面已经定义为 info_excel_path
            # 如果上面没定义变量，请使用 os.path.join(base_path, "info.xlsx")
            
            try:
                # 调用我们在 Step 1 写的模块
                client_check.run_check(info_excel_path, booking_list_path)
                print("✓ 客户核对完成！结果已更新至 info.xlsx")
            except Exception as e:
                print(f"⚠ 警告：客户核对过程中出错: {e}")
        else:
            print("\n【额外步骤】未选择 Booking List 或文件不存在，跳过客户核对。")
        
        # ==================== 额外步骤：自动查价 ====================
        if not price_list_path or not price_list_path.strip():
            print("\n【额外步骤】未选择 Price List，跳过自动查价。")
        elif not os.path.exists(price_list_path):
            print(f"\n【额外步骤】Price List 文件不存在: {price_list_path}，跳过自动查价。")
        else:
            # 检查 info.xlsx 是否存在（如果没有数据，文件可能不存在）
            if not os.path.exists(info_excel_path):
                print(f"\n【额外步骤】info.xlsx 不存在，无法执行自动查价。")
            else:
                print(f"\n【额外步骤】检测到 Price List，开始执行自动查价...")
                print(f"Price List 路径: {price_list_path}")
                ...
                try:
                    ...
                    # 加载运价表
                    matcher.load_price_list(price_list_path)
                    
                    # 获取刚刚生成的结果 Excel 文件路径（info.xlsx）
                    generated_report_path = info_excel_path
                    
                    # 执行价格匹配和回填
                    matcher.run_matching(generated_report_path)
...
class InvoiceAutoGUI:
    ...
    def create_widgets(self):
        ...
        # Booking List 选择区域
        booking_frame = tk.Frame(self.root, pady=10)
        booking_frame.pack(fill=tk.X, padx=10)
        
        tk.Label(booking_frame, text="Booking List (可选)：", font=("Microsoft YaHei", 10)).pack(side=tk.LEFT)
        
        self.booking_label = tk.Label(booking_frame, text="未选择，将跳过核对", 
                                     font=("Microsoft YaHei", 9), fg="gray", anchor="w")
        self.booking_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        
        booking_btn = tk.Button(booking_frame, text="选择文件", command=self.select_booking_list,
                               font=("Microsoft YaHei", 10), bg="#FF9800", fg="white",
                               relief=tk.RAISED, cursor="hand2")
        booking_btn.pack(side=tk.RIGHT)
        
        # Price List 选择区域
        price_list_frame = tk.Frame(self.root, pady=10)
        price_list_frame.pack(fill=tk.X, padx=10)
        
        tk.Label(price_list_frame, text="Price List (可选):", font=("Microsoft YaHei", 10)).pack(side=tk.LEFT)
        
        self.price_list_label = tk.Label(price_list_frame, text="未选择，将跳过自动查价", 
                                        font=("Microsoft YaHei", 9), fg="gray", anchor="w")
        self.price_list_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        
        price_list_btn = tk.Button(price_list_frame, text="选择文件", command=self.select_price_list,
                                  font=("Microsoft YaHei", 10), bg="#9C27B0", fg="white",
                                  relief=tk.RAISED, cursor="hand2")
        price_list_btn.pack(side=tk.RIGHT)
    ...
    def select_booking_list(self):
        ...
        file_path = filedialog.askopenfilename(
            title="选择 Booking List 文件",
            filetypes=[("Excel 文件", "*.xlsx *.xls"), ("所有文件", "*.*")]
        )
        
        if file_path:
            self.booking_list_path = file_path
            # 显示文件名（如果路径太长，只显示文件名）
            display_name = os.path.basename(file_path)
            if len(display_name) > 50:
                display_name = display_name[:47] + "..."
            self.booking_label.config(text=display_name, fg="blue")
            print(f"已选择 Booking List: {file_path}")
        else:
            # 用户取消选择，恢复默认状态
            self.booking_list_path = None
            self.booking_label.config(text="未选择，将跳过核对", fg="gray")
    ...
    def select_price_list(self):
        ...
        file_path = filedialog.askopenfilename(
            title="选择 Price List 文件",
            filetypes=[("Excel 文件", "*.xlsx"), ("所有文件", "*.*")]
        )
        
        if file_path:
            self.price_list_path = file_path
            # 显示文件名（如果路径太长，只显示文件名）
            display_name = os.path.basename(file_path)
            if len(display_name) > 50:
                display_name = display_name[:47] + "..."
            self.price_list_label.config(text=display_name, fg="blue")
            print(f"已选择 Price List: {file_path}")
        else:
            # 用户取消选择，恢复默认状态
            self.price_list_path = None
            self.price_list_label.config(text="未选择，将跳过自动查价", fg="gray")
